	...
	if params[1] == "." {
		...
		if header.Get("Reply-To") == "" {
			...
			if re.MatchString(e.Address) {
				...
				var replyTo string
				if e.Name != "" {
					replyTo = e.Name + " <" + parts[0] + "_" + parts[1] + "@hmail.app>"
				} else {
					replyTo = parts[0] + "_" + parts[1] + "@hmail.app"
				}
				var fromIndex int
				for k := range data[sessionId] {
					if strings.HasPrefix(strings.ToLower(data[sessionId][k]), "from: ") {
						fromIndex = k
					}
				}
				data[sessionId] = append(data[sessionId][:fromIndex+1], data[sessionId][fromIndex:]...)
				data[sessionId][fromIndex] = "Reply-To: " + replyTo
			}
		}
		for k := range data[sessionId] {
